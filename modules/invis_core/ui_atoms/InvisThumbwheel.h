...
/**
 * Stateless Presentational 3D Thumbwheel / Wheel Selector Atom.
 * Edge-on knurled wheel with 3D cylindrical projection, smooth drag rotation, and item snapping.
 */
class InvisThumbwheel : public juce::Component, public InvisThemeSupplier {
public:
    InvisThumbwheel();
    ~InvisThumbwheel() override = default;
...
private:
    ...
    float ledLuminance { 1.0f }; // 1.6 = ignition spike, 1.0 = steady lit state

    bool isPressed { false };

    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(InvisThumbwheel)
};
