...
const EffectParam kSpatialParams[] = {
    { "SIZE",     8.0f,  900.0f, 0.35f, " MS", 0, true  },
    { "FEEDBACK", 0.0f,  100.0f, 0.45f, "%",   0, false },
...
const EffectParam* getAlgorithmParams(AlgorithmKind kind, int& count)
{
    switch (kind)
    {
        ...
        case AlgorithmKind::Spatial:
        default:
            count = static_cast<int>(std::size(kSpatialParams));
            return kSpatialParams;
    }
}
